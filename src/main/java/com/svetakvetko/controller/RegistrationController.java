...
public class RegistrationController {
    ...
    public ModelAndView registerUser(@Validated(RegistrationGroup.class) @ModelAttribute("user") User user, BindingResult bindingResult, Errors errors) {
        ...
        if (userService.isExist(user.getUserLogin())) {
            ...
            userService.create(new User(user.getUserLogin(), passwordEncoder.encode(user.getPassword()), Collections.singletonList(new Role(1, USER_ACCESS.getName())), user.getEmail(), user.getName(), user.getSurname(), user.getSalary(), user.getDateOfBirth()));
